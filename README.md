...
정답으로 받은 **세자리 코드**만 이 웹앱에 입력한다 — 두 시스템은 코드로만 연결되며 API 연동 없음.
...
content/           콘텐츠 — 파일 교체만으로 반영, 코드 수정·재시작 불필요
  ├ dosan/         도안 PDF (A4 세로 · 흑백 라인아트 · 하단 30mm 비움)
...
① 대기존에서 브로슈어 읽기 → ② 퀴즈존에서 스마트폰으로 NFC 피규어 퀴즈 → 세자리 코드 획득
...
어트랙트 → 코드 입력(숫자 키패드, 3자리 자동 검증) → **도안 만들기(프로그램 3종 중 택 1)**
...
## 미니게임 6종 (모두 트랙볼 클릭 전용 · 1~2분 · 호출 시 즉시 중단)
